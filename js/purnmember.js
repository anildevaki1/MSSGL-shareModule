        ...
        if ($stateParams.action)
            ...
            $scope.getMemberRequestdetail=function(){
                if(vm.entity.regCode!=null)
                {
                    ...
                    $(".loading").show();
                    ajax.get('Member/getmemberdata', null,param).then(function (res) {
                        if (res) {
                            vm.entity = res;
                        }
                        else {
            ...
            $scope.save = function (fn) {
                if ($scope.memberform.$valid) {
                    ...
                    if (!vm.entity.regCode)
                        ajax.post('Member/insert', vm.entity).then(function (res) {
                            if (res) {
                        ...
                        ajax.put('Member/update', vm.entity, { id: vm.entity.regCode }).then(function (res) {
                            if (res) {
                                $(".loading").hide();
                                $scope.message = "Record Saved Sucessfully";
                                R1Util.createAlert($scope, "Success", $scope.message, null);
